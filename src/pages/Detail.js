...
const Detail = ({ setActive, user }) => {
  ...
  const getBlogDetail = async () => {
    try {
      ...
      const blogDetailSnapshot = await getDoc(docRef);
      const blogDetailData = blogDetailSnapshot.data();
  
      if (!blogDetailData) {
        // Handle scenario where blog detail doesn't exist
        setLoading(false);
        return;
      }
  
      setBlog(blogDetailData);
      setTags([...new Set(blogDetailData.tags)]);
      setComments(blogDetailData.comments || []);
      setLikes(blogDetailData.likes || []);
  
      const relatedBlogsQuery = query(
        blogRef,
        where("tags", "array-contains-any", blogDetailData.tags),
        limit(3)
      );
